...
public class SessionCookieBodyTests
{
    ...
    {
        HttpClient? loginClient = _factory.CreateClient(new() { HandleCookies = false });
        HttpResponseMessage? login = await loginClient.PostAsJsonAsync("/api/auth/login", new
        {
        ...
        LoginBody? body = await login.Content.ReadFromJsonAsync<LoginBody>(JsonOptions);
        body.Should().NotBeNull();
        body!.AccessToken.Should().NotBeNullOrWhiteSpace();

        HttpClient? api = _factory.CreateClient(new() { HandleCookies = false });
        HttpRequestMessage req = new HttpRequestMessage(HttpMethod.Get, "/api/organizations");
        req.Headers.Authorization = new AuthenticationHeaderValue("Bearer", body.AccessToken);
        HttpResponseMessage? orgs = await api.SendAsync(req);
